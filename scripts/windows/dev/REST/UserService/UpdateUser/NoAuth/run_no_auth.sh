...
curl -X PUT "$BASE_URL/$MOBILE_NUMBER" -H 'Content-Type: application/json' -H "accept: application/json" -d @UpdateUserRequest.json | node -e "s=process.openStdin();d=[];s.on('data',function(c){d.push(c);});s.on('end',function(){console.log(JSON.stringify(JSON.parse(d.join('')),null,2));})" >> "$filename";
