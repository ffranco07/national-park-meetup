...
curl -X POST "$BASE_URL" -H 'Content-Type: application/json' -H "accept: application/json" -d @CreateUserRequest.json | node -e "s=process.openStdin();d=[];s.on('data',function(c){d.push(c);});s.on('end',function(){console.log(JSON.stringify(JSON.parse(d.join('')),null,2));})" >> "$filename";
